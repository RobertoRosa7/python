...
def match_strings(string):
    try:
        ...
        bats_one = (
            re.compile(r"Bat(man|mobile|copter|bat)").search(string).group()
        )  # or
...
def search_number_phone(number):
    ...
    phone_number_space = re.compile(
        r"\(\d\d\) (\d\d\d\d\d-\d\d\d\d)"
    )  # use \(and\) to match literal parenteses
    ...
    try:
        ...
        notification(
            message="Não foi encontrado nenhum número valido de celular\nEx: 11-91234-5678\nAttributeError"
        )
    except TypeError:
        notification(
            message="Não foi encontrado nenhum número valido de celular\nEx: 11-91234-5678\nTypeError"
...
def find_all_expression():
    find_all_any = re.compile(r".at").findall(
        "the cat in the hat sat on the flat mat"
    )  # any character
    find = (
        re.compile(r"^\d+$").search("123424023408").group()
    )  # everything that begins and ends with numbers
    the_first_and_the_last_name = re.compile(
        r"First name: (.*) Last name: (.*)"
    ).findall("First name: Roberto Last name: Rosa")
    search_anything = re.compile(r"<(.*?)>").findall("<Hello> is real>")
    search_with_new_line = (
        re.compile(r".*", re.DOTALL).search("is\nother\nnew line").group()
    )
    search_and_ignore_case = re.compile(r"[aeiou]", re.IGNORECASE).findall(
        "Al, why you does your programming book talk"
    )
