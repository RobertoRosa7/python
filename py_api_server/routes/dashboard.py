...
from py_api_server.enviroment.enviroment import db
from py_api_server.robots.get_status_code import get_status_code
from py_api_server.utils.gets import get_user
  ...
  try:
    ...
    if find_result != None and type(find_result) == dict:
      ...
      if result.modified_count > 0:
        ...
        response = jsonify({'status':200, 'msg':'Um registro foi modificado', 'py_data': json.loads(dumps(data))})
        response.status_code = 200
        return response
      else:
...
def delete_one():
  try:
    ...
    if find_result != None and type(find_result) == dict:
      ...
      data['total'] = len(str_to_json)
      response = jsonify({'status':200, 'msg': 'total de registros: ' + str(len(str_to_json)), 'py_data': data})
      response.status_code = 200
...
#     mode = True if py_data['mode'] == 'dev-mode' else False
