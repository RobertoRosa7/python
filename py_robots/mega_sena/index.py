...
def converte_json_to_csv():
    # dcsv = pd.read_csv(os.path.join(os.getcwd(), 'py_databases/mega.csv'))
    # djson = pd.read_json(os.path.join(os.getcwd(), 'py_databases/mega.json'))
    # dataCsv = dataJson.to_csv(os.path.join(os.getcwd(), 'py_databases/mega.csv'), index=None)
...
def create_csv_mega(payload, filename):
    if not formats.is_file_exists(filename):
        with open(formats.path_database(filename), "a") as new_file:
            ...
            writer.writerow(
                ["py_data", "bola 1", "bola 2", "bola 3", "bola 4", "bola 5", "bola 6"]
            )
...
def convertExcelMegaSenaToCsv(filename):
    try:
        ...
        df.columns = map(str.lower, df.columns)  # tolowercase columns
        df["py_data"] = pd.to_datetime(df["py_data"])
...
dateLast = df.sort_values("py_data", ascending=False)[-1:]["py_data"].values[0]
dateFirst = df.sort_values("py_data", ascending=True)[-1:]["py_data"].values[0]
...
df.loc[
    (df["py_data"] <= dateMediaFirst) & (df["py_data"] >= dateMediaLast), "proba"
] = "moderate"

df.loc[(df["py_data"] > dateMediaFirst) & (df["py_data"] <= dateFirst), "proba"] = "low"
df.loc[(df["py_data"] >= dateLast) & (df["py_data"] < dateMediaLast), "proba"] = "high"

moderate = df.loc[(df["py_data"] <= dateMediaFirst) & (df["py_data"] >= dateMediaLast)]
low = df.loc[(df["py_data"] > dateMediaFirst) & (df["py_data"] <= dateFirst)]
high = df.loc[(df["py_data"] >= dateLast) & (df["py_data"] < dateMediaLast)]
